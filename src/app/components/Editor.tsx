...
import { Toggler, Dropdown } from "./index"
...
const Editor = () => {
  ...
  const handleClick = () => {
    ...
    const textDisplay = toggleOption ? "Replace text" :  "Paste outside text" 
    parent.postMessage(
      {
        pluginMessage: {
  ...
  return (
    <div className='flex flex-col justify-start justify-self-start gap-6 w-full px-6 pt-9'>
        ...
        <button className={`flex items-center justify-center w-full text-xs ${loading ? 'bg-[#979797] gap-3' : 'bg-textDark'} text-white text-center p-3 mt-4`} onClick={handleClick}>
        {loading && (<svg role="status" className="inline w-4 h-4 text-white animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          {loading? 'Rewriting Text' : 'Rewrite Text'}
        </button>
